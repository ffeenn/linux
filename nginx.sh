...
yum -y install unzip gcc gcc-c++ wget gd-devel openssl openssl-devel
...
Vhost_Path=$Server_Path/vhost
Setup_Path=$Server_Path/nginx
run_path='/usr/local/src'
PHP_Pash=$Server_Path/php
...
Install_Nginx()
{
	...
	sed -i "s#include /www/server/panel/vhost/nginx/\*.conf#include ${Vhost_Path}/nginx/\*.conf;#" ${Setup_Path}/conf/nginx.conf
	sed -i "s#/www/server/phpmyadmin#${Server_Path}/phpmyadmin#" ${Setup_Path}/conf/nginx.conf
	sed -i "s#/www/server#${Server_Path}#" ${Setup_Path}/conf/nginx.conf
	sed -i "s#/www/wwwlogs#${Server_Path}/wwwlogs#" ${Setup_Path}/conf/nginx.conf
    ...
    wget -O /etc/init.d/nginxd ${download_Url}/init/nginx.init -T 5
	sed -i "s#/www/server#${Server_Path}#" /etc/init.d/nginxd
	...
	cat > ${Vhost_Path}/vhost/nginx/phpfpm_status.conf<<EOF
server {
	listen 80;
...
lua_package_path "${Setup_Path}/nginx/waf/?.lua";
init_by_lua_file  ${Setup_Path}/nginx/waf/init.lua;
access_by_lua_file ${Setup_Path}/nginx/waf/waf.lua;
EOF
	
	rm -f ${Setup_Path}/conf/vhost/default.conf
	mkdir -p ${Server_Path}/wwwlogs/waf
	chown www.www ${Server_Path}/wwwlogs/waf
	chmod 744 ${Server_Path}/wwwlogs/waf
	wget -O waf.zip ${download_Url}/install/waf/waf.zip
	unzip -o waf.zip -d $Setup_Path/ > /dev/null
	if [ ! -d "${Vhost_Path}/wafconf" ];then
		mv $Setup_Path/waf/wafconf ${Vhost_Path}/wafconf
	fi
	cd ${Setup_Path}
	rm -f src.tar.gz
	CheckPHPVersion
	...
	echo "${nginxVersion}" > ${Setup_Path}/version.me
}
Update_nginx(){
	cd ${run_path}
	...
	nginx -v
	echo "$nginxVersion" > ${Setup_Path}/version.me
	rm -f ${Setup_Path}/version_check.me
}
CheckPHPVersion()
{
	PHPVersion=""
...
if [ "$actionType" == 'install' ];then
	...
	Install_Nginx
	if [ ! -f $Setup_Path/nginx/conf/enable-php-00.conf ];then
		echo > $Setup_Path/nginx/conf/enable-php-00.conf
	fi
	
	if [ -f $Setup_Path/vhost/nginx/btwaf.conf ];then
		echo > $Setup_Path/nginx/conf/luawaf.conf
	fi
